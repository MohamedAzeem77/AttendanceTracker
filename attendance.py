...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:azeem@localhost:5432/newattendance1'
...
jwt = JWTManager(app)
mail = Mail(app)
swagger = Swagger(app, template_file='E:\PycharmProjects\pythonProject\.venv\Scripts\Attendance\swagger_doc\swagger.yaml')
...
def generate_swagger_yaml():
    ...
    for rule in app.url_map.iter_rules():
        if rule.endpoint != 'static':
            paths[str(rule)] = {
                'get': app.view_functions[rule.endpoint].__doc__,
                'post': app.view_functions[rule.endpoint].__doc__,
                'put': app.view_functions[rule.endpoint].__doc__,
                'delete': app.view_functions[rule.endpoint].__doc__,
            }
